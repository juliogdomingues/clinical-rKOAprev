...
The objectives of this study were to develop a parsimonious model for identifying prevalent radiographic knee osteoarthritis from characteristics that are not themselves manifestations of the disease; to compare its discrimination and calibration with those of three machine-learning algorithms under an identical validation procedure; and to quantify the incremental contribution of self-reported symptoms, which are manifestations of the disease, and of bioimpedance-derived body composition.
...
Three sets of candidate variables were specified in advance, and the separation between the first two is central to the analysis. Knee symptoms are a manifestation of osteoarthritis rather than a characteristic of the person that exists independently of it. A model containing symptoms therefore addresses a different question from one that does not: it describes how well structural disease can be classified once the clinical presentation is already known, whereas a model without symptoms describes how far structural status is reflected in characteristics that are not themselves expressions of the disease. Combining the two would conflate these questions and would leave the contribution of symptoms unquantified.

Accordingly, the **Constitutional set** comprised demographic, anthropometric, clinical-history and occupational variables, that is, characteristics of the individual that do not depend on current symptom status. The **Symptom-Augmented set** added the three symptom items, allowing their incremental contribution to be estimated as the difference between the two. A third set additionally included the bioimpedance-derived measures of skeletal muscle mass, bone mineral content and mineral mass, which were otherwise withheld, in order to quantify their incremental contribution in the same way.

One variable in the Constitutional set requires comment. A history of knee surgery is not a symptom, but neither is it wholly independent of the disease, since surgery is frequently undertaken because of established joint pathology. It was retained because it is routinely recorded and is not a report of current symptoms, and its influence was examined directly in a sensitivity analysis in which it was removed.
...
Discrimination was quantified by the area under the receiver operating characteristic curve computed on the pooled held-out estimates. Calibration was assessed by the calibration slope, obtained as the coefficient of the linear predictor when the observed outcome is regressed upon it [Van Calster et al.]. The ideal value is 1; a slope below 1 indicates that estimated probabilities are too extreme, and a slope above 1 that they are too conservative. Agreement across the range of estimates was examined graphically by plotting observed proportions against model-estimated probabilities by decile. The Brier score was reported as an overall measure.
...
The logistic model was well calibrated in both variable sets. In the Constitutional set the calibration slope was 0.95 (95% CI 0.87, 1.04), an interval including the value denoting agreement, and observed proportions followed model-estimated probabilities across the range of estimates (Supplementary Figure S1). The Brier score was 0.098. Among the comparators, extreme gradient boosting was similarly calibrated (1.05; 0.96, 1.14), whereas the random forest produced estimates that were insufficiently dispersed (1.16; 1.05, 1.26) and the multilayer perceptron estimates that were too extreme (0.79; 0.70, 0.89). The algorithm with the lowest discrimination was therefore also the least well calibrated.
